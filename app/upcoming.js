...
checkbox.addEventListener('change', () => {
    let eventosFiltrados = upcomingEvents
    let checkboxChecked = document.querySelectorAll("input[type=checkbox]:checked")
    
    if (checkboxChecked.length != 0) {
        eventosFiltrados = filtrarCheck(checkboxChecked, upcomingEvents)
    }
...
buscar.addEventListener('input', (filtroBusqueda) => {
    let checkboxChecked = document.querySelectorAll("input[type=checkbox]:checked")
    let texto = document.getElementById("busqueda").value
    let eventosFiltrados2 = filtrarText(texto, upcomingEvents)
    if (checkboxChecked.length != 0) {
...
function filtrarCheck(arregloEventos, arreglo) {
    ...
    let eventosFiltrados = arreglo.filter(upcomingEvents => arregloEventos.includes(upcomingEvents.category))
    return eventosFiltrados
}

function filtrarText(texto, arreglo) {
    let eventosFiltrados2 = arreglo.filter(upcomingEvents => upcomingEvents.name.toLowerCase().includes(texto.toLowerCase()))
